...
DWORD WINAPI _CreateAmbientFx1(LPVOID lpParameter)
{
    ...
    while(TRUE)
    {
        ...
        if(DeviceType == 1)
        {
            StretchBlt(hMemoryDC, 0, 0, ChromaSDK::Keyboard::v2::MAX_COLUMN, ChromaSDK::Keyboard::v2::MAX_ROW, 
        ...
        if(DeviceType == 1)
        {
            bmi.biWidth = ChromaSDK::Keyboard::v2::MAX_COLUMN;
            bmi.biHeight = ChromaSDK::Keyboard::v2::MAX_ROW;
        }
        else if(DeviceType == 5)
        {
        ...
        if(DeviceType == 1)
        {
            ...
            for(UINT i=0; i<ChromaSDK::Keyboard::v2::MAX_ROW; i++)
            {
                ...
                for(UINT j=0; j<ChromaSDK::Keyboard::v2::MAX_COLUMN; j++)
                {
                    // Fill up the array
