...
BOOL CHealthAmmoManaDlg::OnInitDialog()
{
    ...
    if (m_DeviceType == 4) //headset
    {
        GetDlgItem(IDC_SLIDER_AMMO_MANA)->ShowWindow(SW_HIDE);
        GetDlgItem(IDC_AMMO)->ShowWindow(SW_HIDE);
        GetDlgItem(IDC_HEALTH)->SetWindowTextW(L"Health/Ammo/Mana");
    }
    ...
    case 6: // Chroma Linked devices
        {
           ...
           /* for(UINT i=0; i<RemainingHealth; i++)
            ...
            Effect.Color[0] = HpColor;
            
            UINT RemainingAmmo = UINT(Ammo / 100.0 * 5.0); // 5 keys.
            for (UINT i = 0; i<RemainingAmmo; i++)
            {
                if(i > 0)Effect.Color[i] = YELLOW;
            }
