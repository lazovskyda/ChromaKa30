...
void CMousematPage::OnBnClickedButtonLoadingAnimation()
{
    ...
    if(IsEqualGUID(EffectId, GUID_NULL))
    {
        ...
        for(int i=0; i<ChromaSDK::Mousepad::v2::MAX_LEDS; i++)
        {
            Effect.Color[ChromaSDK::Mousepad::v2::MAX_LEDS-1-i] = ORANGE;

            g_ChromaSDKImpl.CreateMousematEffectImpl(ChromaSDK::Mousepad::CHROMA_CUSTOM2, &Effect, &Frames[i+1]);
